    ...
    public File generateAndSaveCode(String userMessage, CodeGenTypeEnum codeGenTypeEnum) {
        ...
        return switch (codeGenTypeEnum) {
            case HTML -> {
                ...
                yield CodeFileSaverExecutor.executeSaver(CodeGenTypeEnum.HTML,result);
            }
            case MULTI_FILE -> {
                MultiFileCodeResult result = aiCodeGeneratorService.generateMultiFileCode(userMessage);
                yield CodeFileSaverExecutor.executeSaver(CodeGenTypeEnum.MULTI_FILE,result);
            }
            default -> {
    ...
    private Flux<String> processCodeStream(Flux<String> codeStream, CodeGenTypeEnum codeGenType) {
        ...
        return codeStream.doOnNext(chunk -> {
            ...
            try {
                ...
                // 使用执行器保存代码
                File savedDir = CodeFileSaverExecutor.executeSaver(codeGenType,parsedResult);
